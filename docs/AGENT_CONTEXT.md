...
Email notifications are handled by `src/lib/email-notifications.ts` and called from `submitContactFormAction()` after both enquiry and blocked records are stored. Studio settings at `/studio/settings` let an admin set one or more recipient inboxes, comma-separated, and choose mode: `immediate`, `daily_digest`, `weekly_digest`, or `off`; the save action validates recipient email shape before writing to Neon. Delivery uses the official `resend` package and needs `RESEND_API_KEY` plus `CONTACT_NOTIFICATION_FROM` in Vercel; never hard-code an `re_...` key in source. The temporary live recipient is `jakubgajosz1999@gmail.com` because Resend allows `onboarding@resend.dev` only to the account email before domain verification. The Resend key is configured in Vercel, `CONTACT_NOTIFICATION_TO` is temporarily set to the Gmail address, and the Studio notification setting in Neon is also set to that Gmail address. The first-email test and a live contact enquiry test both completed; the onboarding email landed in spam, which is acceptable for owner-only testing. Before public launch, verify the Eltronic domain in Resend and switch to a sender such as `notifications@eltronic.co.uk`, then change the recipient back to the preferred owner inbox if needed. Use `npm run email:check` to verify env vars, `npm run email:check -- --send --onboarding` for the Resend first-email test, and `npm run email:check -- --send` to send a real configured notification test after credentials are configured.
