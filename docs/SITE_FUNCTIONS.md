...
- Resend delivery uses the official `resend` package and requires `RESEND_API_KEY` plus `CONTACT_NOTIFICATION_FROM`; the default recipient is `jakub@gajosz.com`. The current Vercel key is installed, but Resend requires a verified sending domain before the temporary sender can deliver to `jakub@gajosz.com`. The onboarding test delivered to `jakubgajosz1999@gmail.com` but landed in spam, so launch email should use a verified Eltronic domain sender. `npm run email:check -- --send --onboarding` sends the Resend first-email test to the account email.
