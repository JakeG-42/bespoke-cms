...
async function getPayloadPage(slug: string): Promise<Page | null> {
  try {
    ...
    console.error("Unable to load Payload preview page.", error);
    return null;
  }
}

export async function generateMetadata({ params }: PayloadPreviewPageProps): Promise<Metadata> {
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `https://app.example.com/preview${getPagePath(slug)}`,
    },
    robots: {
      index: false,
      follow: false,
...
async function getFeaturedProducts(): Promise<Product[]> {
  try {
    ...
    console.error("Unable to load Payload products for preview.", error);
    return [];
  }
}
...
async function getMenus(): Promise<BuilderMenu[]> {
  try {
    ...
    console.error("Unable to load Payload menus for preview.", error);
    return [];
  }
}
...
async function getThemes(): Promise<BuilderTheme[]> {
  try {
    ...
    console.error("Unable to load Payload themes for preview.", error);
    return [];
  }
}
...
async function getThemeSettings(): Promise<BuilderThemeSettings> {
  try {
    ...
    console.error("Unable to load Payload theme settings for preview.", error);
    return {};
  }
}
...
export default async function PayloadPreviewPage({ params }: PayloadPreviewPageProps) {
  const { slug: segments } = await params;
  const slug = getSlugFromSegments(segments);
